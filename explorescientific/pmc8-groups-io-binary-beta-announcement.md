...
This beta release is therefore intended to validate the updated driver before it is submitted upstream. It is still beta software. Please test only if you are comfortable replacing a driver executable, restoring your previous driver if needed, and collecting logs.
...
Each beta artifact includes:

- `indi_pmc8_telescope`
- a matching `indiserver`
...
These artifacts are not complete KStars distributions. They provide a matching beta INDI server/runtime layer plus the PMC-Eight telescope driver executable.
...
Use this artifact on a normal Linux PC or notebook with an x86_64 processor.
...
Raspberry Pi note: this beta artifact is intended for 64-bit Raspberry Pi systems. It is not intended for older 32-bit Raspberry Pi OS installations.
